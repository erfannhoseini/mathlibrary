...
Array.prototype.sliceShape=function(v1,v2,v3,v4,f) {
  ...
  for (let j=0;j<this.length; j++) { // this loop for check all shape if cuting line
    ...
    for (let i=0; i<this[j].length-1; i++) {
      let m1 = Parangle(this.x1,this.y1,this.x2,this.y2, 1  );
      let m2 = Parangle(limitFloat(this[j][i].x, 2), limitFloat(this[j][i].y, 2), limitFloat(this[j][i+1].x, 2), limitFloat(this[j][i+1].y, 2),0);
      ...
      if ( !( a1==a2 || a1==a2-180 || a1==a2+180 ) ) { 
        if (inte(m1,m2)) {
          ...
          if(f){console.log(checkLinesObj({x1:this[j][i].x,y1:this[j][i].y,x2:this[j][i+1].x,y2:this[j][i+1].y},this[j]));}
        } 
      }
    }
    ...
    if (cpa.length>0) {
      ...
      if ( cpa.length==1) {
        ...
        if(f){console.log(checkInsideObj(this.x1,this.y1,this[j]),checkInsideObj(this.x2,this.y2,this[j]));}
        if (checkInsideObj(this.x1,this.y1,this[j])) {
          ...
          for(let hg=0;hg<cpa.length;hg++){
             ...
             if(dist1==dist(cpa[hg].p.x,cpa[hg].p.y,this.x1,this.y1) && cpa[numdist1].idxline(this[j]) > cpa[hg].idxline(this[j]) ){
                numdist1=hg;
             }
             
          }
          
          dist1 =dist(cpa[0].p.x,cpa[0].p.y,this.x1,this.y1);
          for(let hg=0;hg<cpa.length;hg++){
             if(dist1>dist(cpa[hg].p.x,cpa[hg].p.y,this.x1,this.y1)){
               dist1=dist(cpa[hg].p.x,cpa[hg].p.y,this.x1,this.y1);
               numdist2=hg;
        ...
        if (checkInsideObj(this.x2,this.y2,this[j])) {
          ...
          for(let hg=0;hg<cpa.length;hg++){
             ...
             if(dist1==dist(cpa[hg].p.x,cpa[hg].p.y,this.x2,this.y2) && cpa[numdist1].idxline(this[j]) > cpa[hg].idxline(this[j]) ){
                numdist1=hg;
             }
             
          }
          
          dist1 =dist(cpa[0].p.x,cpa[0].p.y,this.x2,this.y2);
          for(let hg=0;hg<cpa.length;hg++){
             if(dist1>dist(cpa[hg].p.x,cpa[hg].p.y,this.x2,this.y2)){
               dist1=dist(cpa[hg].p.x,cpa[hg].p.y,this.x2,this.y2);
               numdist2=hg;
          ...
          n2 = checkLinesObj(cpa[numdist1].l,this[j]); 
          ...
          if (a2<a1&&a2>a1-180) {
            ...
            if(f){console.log("b2");}  //cpa.length-1
            n2 =checkLinesObj(cpa[numdist2].l,this[j]);
        ...
        //if(cpa.length>3){
        //for(let bm=0;bm<cpa.length-1;bm++){
        //  if(cpa[bm].p.x==cpa[bm+1].p.x&&cpa[bm].p.x==cpa[bm+1].p.x){
        //   //if(
        //  } 
        //}
        //}
          ...
          //n1=0; 
          //for(let hg=0;hg<cpa.length;hg++){
          //  if(n2>checkLinesObj(cpa[hg].l,this[j])){
          //    n2=checkLinesObj(cpa[hg].l,this[j]);
          //    n1 = hg;
          //  }
          //}
        ...
        for (let mn=0; mn< cpa.length   ; mn+=2) {
          ...
          for(let jn=0;jn<this.length;jn++){
            ...
            if(checkLinesObj(cpa[mn].l,this[jn])||checkLinesObj(cpa[mn+1].l,this[jn])){
              idxobj=jn; 
              if(f){console.log(checkLinesObj(cpa[mn].l,this[jn]),checkLinesObj(cpa[mn+1].l,this[jn]));}
            }
          }
...
function checkInsideObj(x, y, a) {
  ...
  this.bol3=false;
  this.bol4=false;
  this.l1 =Parangle(x, y, 0 , y-0.5 , 0);
  this.l2 =Parangle(x, y, dm, y+0.5 , 0);
  this.l3 =Parangle(x, y, x+0.5 , 0 , 0);
  this.l4 =Parangle(x, y, x-0.5 , dm, 0);
